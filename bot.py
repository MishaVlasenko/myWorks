...
def lalala(message):
    if message.chat.type == 'private':
        if message.text == 'Рандомное число':
            ...
            markup = types.InlineKeyboardMarkup(row_width=2)
            item1 = types.InlineKeyboardButton("Я тоже хочу", callback_data='1')
            item2 = types.InlineKeyboardButton("Не пью", callback_data='2')
 
            markup.add(item1, item2)
            bot.send_message(message.chat.id,'Пью пиво',reply_markup=markup)
    ...
    try:
        if call.message:
            if call.data == 'good':
                ...
                bot.send_message(call.message.chat.id, 'Пойди купи')
